...
public class BoatAutoPilot : MonoBehaviour
{
    ...
    private void Update()
    {
        Collider[] colliders = Physics.OverlapSphere(transform.position, boatSettings.neighborhoodRadius);
    ...
    private Vector3 ComputeAcceleration(IEnumerable<BoatAutoPilot> boats)
    {
        ...
        acceleration += ComputeAlignment(boats) * boatSettings.alignmentAmount;
        acceleration += ComputeSeparation(boats) * boatSettings.separationAmount;
        acceleration += ComputeCohesion(boats) * boatSettings.cohesionAmount;
    ...
    private void UpdateVelocity(Vector3 acceleration)
    {
        ...
        velocity = LimitMagnitude(velocity, boatSettings.maxSpeed);
    }
    ...
    private void UpdateRotation(Vector3 velocity)
    {
        ...
        transform.forward = Vector3.RotateTowards(transform.forward, velocity, Time.deltaTime * boatSettings.steeringSpeed, float.MaxValue);
    }
    ...
    private Vector3 ComputeAlignment(IEnumerable<BoatAutoPilot> boats)
    {
        ...
        var steer = Steer(velocity.normalized * boatSettings.maxSpeed);
        return steer;
    }
    ...
    private Vector3 ComputeCohesion(IEnumerable<BoatAutoPilot> boats)
    {
        ...
        var steer = Steer(direction.normalized * boatSettings.maxSpeed);
        return steer;
    }
    ...
    private Vector3 ComputeSeparation(IEnumerable<BoatAutoPilot> boats)
    {
        ...
        boats = boats.Where(boat => Vector3.Distance(transform.position, boat.transform.position) <= boatSettings.separationRadius);
    ...
    private Vector3 Steer(Vector3 desiredVelocity)
    {
        ...
        steer = LimitMagnitude(steer, boatSettings.maxForce);
        return steer;
    }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Gizmos.DrawWireSphere(transform.position, boatSettings.neighborhoodRadius);

        // Separation radius.
        Gizmos.color = Color.salmon;
        Gizmos.DrawWireSphere(transform.position, boatSettings.separationRadius);
    }
}